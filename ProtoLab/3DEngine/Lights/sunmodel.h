...
Math::SampledSpectrum computeSunRadiance(float theta, float turbidity) {
    Math::SampledSpectrum k_oCurve = Math::SampledSpectrum::FromSampled(k_oWavelengths, k_oAmplitudes, 64);
   	Math::SampledSpectrum k_gCurve = Math::SampledSpectrum::FromSampled(k_gWavelengths, k_gAmplitudes, 4);
    Math::SampledSpectrum k_waCurve = Math::SampledSpectrum::FromSampled(k_waWavelengths, k_waAmplitudes, 13);
    Math::SampledSpectrum solCurve = Math::SampledSpectrum::FromSampled(solWavelengths, solAmplitudes, 38);

    Math::SampledSpectrum sunRadiance{};

    float beta = 0.04608365822050f * turbidity - 0.04586025928522f;

	// Relative Optical Mass
    float m = 1.0f / (std::cos(theta) + 0.15f *
		std::pow(93.885f - theta/HLSL::PI*180.0f, (float) -1.253f));

	int i = 0;
    for(i = 0; i < Math::NumSpectralSamples; ++i) {
        float lambda = lerp(float(Math::SampledLambdaStart), float(Math::SampledLambdaEnd), i / float(Math::NumSpectralSamples));
		...
		float tauA = std::exp(-m * beta * std::pow(lambda/1000.0f, -alpha));  // lambda should be in um

		// Attenuation due to ozone absorption
		// lOzone - amount of ozone in cm(NTP)
		// Results agree with the graph (pg 128, MI)
		const float lOzone = .35f;
		float tauO = std::exp(-m * k_oCurve[i] * lOzone);

		// Attenuation due to mixed gases absorption
		// Results agree with the graph (pg 131, MI)
		float tauG = std::exp(-1.41f * k_gCurve[i] * m / std::pow(1 + 118.93f
			* k_gCurve[i] * m, (float) 0.45f));

		// Attenuation due to water vapor absorbtion
		// w - precipitable water vapor in centimeters (standard = 2)
		// Results agree with the graph (pg 132, MI)
		const float w = 2.0;
		float tauWA = std::exp(-0.2385f * k_waCurve[i] * w * m /
				std::pow(1 + 20.07f * k_waCurve[i] * w * m, (float) 0.45f));

		sunRadiance[i] = solCurve[i] * tauR * tauA * tauO * tauG * tauWA;
    }
