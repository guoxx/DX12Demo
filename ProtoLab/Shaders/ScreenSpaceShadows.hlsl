...
void ShadowMask(float3 positionWS, out int cascadeIdx, out float shadowMask)
{
    // TODO: don't hardcore the size
    float2 shadowMapsSize = float2(2048, 2048);

    float3 shadowPos[4];
    float shadowPosValid[4];
    cascadeIdx = 3;
    ...
    if (SplitMax > 0)
    {
        int idx = SplitPowLookup[SplitMax - 1];
        if (shadowPosValid[idx] > 0)
            cascadeIdx = idx;
    }
...
float4 PSMain(VSOutput In) : SV_TARGET
{
    ...
    int idx;
    float shadowing;
    ShadowMask(positionWS.xyz, idx, shadowing);
