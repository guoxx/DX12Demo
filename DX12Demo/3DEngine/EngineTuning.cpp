...
void HandleDigitalButtonPress( GameInput::DigitalInput button, float timeDelta, std::function<void ()> action )
{
	...
	if (std::floorf(durationHeld * timeStretch) > std::floorf(oldDuration * timeStretch))
		action();
}
...
void EngineTuning::Display( DX12GraphicContext& Context, float x, float y, float w, float h )
{
	...
	//Context.SetScissor((uint32_t)std::floorf(x), (uint32_t)std::floorf(y), (uint32_t)std::ceilf(x + w), (uint32_t)std::ceilf(y + h));
