...
HLSL_CB_DECL(PointLight, Constants, 0,
{
	float4x4 mInvView;
	float4x4 mInvProj;
...
float4 PSMain(VSOutput In) : SV_TARGET
{
	...
	int face = GetFaceOfPointLightShadowMap(HLSL_CB_GET(0, LightPosition).xyz, gbuffer.Position);
	float4 shadowPos = mul(float4(gbuffer.Position, 1), HLSL_CB_GET(0, mLightViewProj[face]));
	...
	float3 L = normalize(HLSL_CB_GET(0, LightPosition).xyz - gbuffer.Position);
	float3 V = normalize(HLSL_CB_GET(0, CameraPosition).xyz - gbuffer.Position);
	float3 N = gbuffer.Normal;
	float NdotL = saturate(dot(N, L));

	float dist = length(HLSL_CB_GET(0, LightPosition).xyz - gbuffer.Position);
	float radiusStart = HLSL_CB_GET(0, LightRadius).x;
	float radiusEnd = HLSL_CB_GET(0, LightRadius).y;
	float3 E = PointLightIrradiance(HLSL_CB_GET(0, LightIntensity).xyz, dist, radiusStart, radiusEnd) * NdotL * PI;
	//if (any(E))
	{
